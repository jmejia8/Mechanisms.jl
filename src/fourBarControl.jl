...
function fourBarSpringDamping(x::Vector{Float64},t::Float64, K::Vector{Float64})
    # four-Bar mechanism with spring and damping control
...
function modelovolt(Kss)
    ...
    #15 de junio de 2012 (MATLAB version)
    # ----------------------------------------------
    # Adapted by jmejia8, April, 2018
    # ----------------------------------------------
    ...
    t, x = myode45(fourBarSpringDamping,TSPAN,x0,KC)  #Solucion del sistema dinamico
...
function merito1(KC::Vector{Float64})
    ...
    t, x = myode45(fourBarSpringDamping,TSPAN,x0,KC) 
...
function getDynamicOptimizationProblem()
    D = 3

    return D, K -> merito1(K)
end